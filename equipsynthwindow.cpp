...
void EquipSynthWindow::drawEntry(QPainter& painter, const SynthEntry& entry, int x, int y, int width)
{
    ...
    // 结果装备图标（名称左侧）
    const QPixmap& resultIcon = equipIcon(entry.iconFile.toLatin1().constData(), 18);
    painter.drawPixmap(x + 12, y + 5, resultIcon);
    ...
    painter.drawText(x + 36, y + 19, entry.displayName);
    ...
    // 合成公式：材料图标 + 箭头 + 结果图标
    const QPixmap& ing1 = equipIcon(entry.ingIcon1.toLatin1().constData(), 16);
    const QPixmap& ing2 = equipIcon(entry.ingIcon2.toLatin1().constData(), 16);
    int rx = x + 14;
    painter.drawPixmap(rx, y + 34, ing1);
    rx += 20;
    ...
    painter.drawText(rx, y + 47, "+");
    rx += 12;

    painter.drawPixmap(rx, y + 34, ing2);
    rx += 20;

    painter.setPen(QColor(255, 180, 40));
    painter.drawText(rx, y + 47, QString::fromUtf8("\342\206\222")); // →
    rx += 16;

    const QPixmap& resultBig = equipIcon(entry.iconFile.toLatin1().constData(), 20);
    painter.drawPixmap(rx, y + 32, resultBig);
    ...
    painter.drawText(x + 14, y + 28, QString::fromUtf8("\350\212\261\350\264\271300\351\207\221\345\270\201")); // 花费300金币
    ...
    painter.drawText(x + 14, y + 68, entry.effects);
}
...
void EquipSynthWindow::paintEvent(QPaintEvent* event)
{
    ...
    struct { QString name; QString icon; QString effect; QColor color; } legends[] = {
        {QString::fromUtf8("Iron Sword"), "upg_dagger.png", QString::fromUtf8("\346\224\273\345\207\273+30"), QColor(210, 100, 30)},            // 剑, 攻击+30
        {QString::fromUtf8("Chain Mail"), "armor.png", QString::fromUtf8("HP+150"), QColor(100, 140, 210)},                                     // 甲, HP+150
        {QString::fromUtf8("Speed Gloves"), "speed_gloves.png", QString::fromUtf8("\346\224\273\351\200\237\303\2270.8"), QColor(180, 160, 40)}, // 攻速, 攻速×0.8
        {QString::fromUtf8("Blue Crystal"), "upg_wand.png", QString::fromUtf8("\346\263\225\345\212\233\346\266\210\350\200\227\303\2270.5"), QColor(130, 80, 210)}, // 魔法石, 法力消耗×0.5
        {QString::fromUtf8("Warhorse"), "warhorse.png", QString::fromUtf8("\345\260\204\347\250\213+1"), QColor(80, 170, 140)},                  // 战马, 射程+1
    };

    int lx = 110;
    for (auto& l : legends) {
        const QPixmap& icon = equipIcon(l.icon.toLatin1().constData(), 16);
        painter.drawPixmap(lx, legendY - 16, icon);
